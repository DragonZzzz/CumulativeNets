...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    label_crop = tf.image.resize_images(label, [crop_h, crop_w])
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror, ignore_label, img_mean): # optional pre-processing arguments
    ...
    # Extract mean.
    img -= img_mean
    label_ = tf.image.decode_png(label_contents, channels=1)
    # Labels needs to be 1 or 0 (edge pixel or not)
    # one = tf.ones_like(label_)
    # b = tf.constant([255])
    # label = tf.div(tf.cast(label_, tf.float32), tf.cast(tf.multiply(one, tf.cast(b, tf.uint8)), tf.float32))
    one = tf.ones_like(label_)
    zero = tf.zeros_like(label_)
    label = tf.where(tf.less(label_, 50), x=zero, y=one)
