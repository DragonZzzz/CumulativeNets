...
class Network(object):
    ...
    def __init__(self, inputs, trainable=True, is_training=False, num_classes=1):
    ...
    def deconv(self,
        ...
        assert c_o % group == 0
        # Convolution for a given input and kernel
        convolve = lambda i, k: tf.nn.conv2d_transpose(i, k, tf.stack([shape[0], shape[1]*m_o, shape[2]*m_o, c_o]),[1, s_h, s_w, 1], padding=padding)
        with tf.variable_scope(name) as scope:
    ...
    @layer
    def upsample(self, input, data, name):
        return tf.image.resize_bilinear(input, [tf.shape(data)[1], tf.shape(data)[2]], name=name)
